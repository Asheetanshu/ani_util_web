...
async function check_input(event){
  ...
  if (uname === "" || passwd === ""){
    alert("Error: Please fill out both username and password.");
    return;
  }
  else if (!is_valid_uname(uname)){
    alert(`Error: Username should only contain\n-> a to z\n-> A to Z\n-> 0 to 9\n{char limit : 16}`);
    return;
  }
  else if (!is_valid_passwd(passwd)){
    alert(`Error: Password should only contain\n-> a to z\n-> A to Z\n-> 0 to 9\nOr Special charecters "!@#$%^&*"\n{char limit : 32}`);
    return;
  }else {
    ...
    try{
      ...
      console.log("WE got something <(*) _ (*)>\n:", reply);
